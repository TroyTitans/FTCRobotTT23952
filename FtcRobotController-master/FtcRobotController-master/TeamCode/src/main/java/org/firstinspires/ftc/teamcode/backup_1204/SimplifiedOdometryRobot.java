...
public class SimplifiedOdometryRobot {
    ...
    private DcMotor rearleft;      //  control the left back drive wheel
    private DcMotor rearright;     //  control the right back drive wheel
    ...
    public void initialize(boolean showTelemetry)
    {
        ...
        rearleft  = setupDriveMotor( "rearleft", DcMotor.Direction.REVERSE);
        rearright = setupDriveMotor( "rearright",DcMotor.Direction.FORWARD);
    ...
    public void turnRobot(double turnAngle, double power) {
        double holdTime = 0;
        double headingDeg = rawHeading + turnAngle;
        yawController.reset(headingDeg, power);

        ElapsedTime stopTime   = new ElapsedTime();
        stopTime.reset();

        myOpMode.telemetry.addData("Axes RH:H:HD", "%5.2f %5.2f %5.2f", rawHeading, turnAngle, headingDeg);
        while (myOpMode.opModeIsActive() && readSensors()) {
    ...
    public void specimenMove(double driveDistanceInches, double strafeDistanceInches, double power, double holdTime) {

        //double holdTime = 0;
        ElapsedTime stopTime   = new ElapsedTime();
        stopTime.reset();
        resetOdometry();
        double strafePower = strafeDistanceInches/driveDistanceInches;

        driveController.reset(driveDistanceInches, power);             //  Maintain zero drive drift
        strafeController.reset(strafeDistanceInches, power);  // Achieve desired Strafe distance
        yawController.reset();                          // Maintain last turn angle
        holdTimer.reset();
    ...
    public void setDriveAccel(double driveAccell)
    {
        driveController.setDriveAccel(driveAccell);
