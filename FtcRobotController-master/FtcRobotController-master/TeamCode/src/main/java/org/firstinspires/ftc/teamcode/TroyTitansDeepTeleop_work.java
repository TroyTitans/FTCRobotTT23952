...
public class TroyTitansDeepTeleop extends LinearOpMode
{
    ...
    final double HEADING_HOLD_TIME  = 1.0 ; // How long to hold heading once all driver input stops. (This Avoids effects of Gyro Drift)
...
*/
        while (opModeIsActive())
        {
            ...
            /*while (slider.isBusy() && (slider.getCurrentPosition() != slider.getTargetPosition()) ) {
                // Wait For Slider to Finish
                slider.setPower(1);
                telemetry.addData("Slider Current", slider.getCurrentPosition(),runtime.milliseconds());
                telemetry.update();
                telemetry.addData("Slider Target", slider.getCurrentPosition(),runtime.milliseconds());
                telemetry.update();
            }*/
            ...
            telemetry.addData("Slider target Position", slider.getTargetPosition());

            telemetry.addData("before manual if", manualSlider);
            if((slider.getCurrentPosition() == slider.getTargetPosition())) {
                telemetry.addData("inside manual if", manualSlider);
                telemetry.update();
               // manualSlider = false;
                slider.setPower(0);
            ...
            {
                telemetry.addData("outside manual if", manualSlider);
                telemetry.update();
            }
...
/*            telemetry.addData("FL Position", frontleft.getCurrentPosition());
    ...
    private void Manual_Mode_Slider() {
        ...
        while (Math.abs(gamepad2.left_stick_y) != 0  && runtime.milliseconds() <= 2000) {
               slider.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            //if (gamepad2.left_stick_y > 0) {
            slider.setPower(-gamepad2.left_stick_y);
            manualSlider = true;
            slider.setTargetPosition(slider.getCurrentPosition());
        //    telemetry.addData("Slider Manual Mode", slider.getCurrentPosition());
          //  telemetry.update();
      //      telemetry.addData("Slider Manual Power", -gamepad2.left_stick_y);
           // telemetry.update();
         /*   } else {
                break;
            }*/
       ...
       /* while (gamepad2.left_stick_y < 0 && runtime.milliseconds() <= 2000) {
        ...
        if(manualSlider) {
           ...
           // telemetry.update();
            slider.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
            slider.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            telemetry.addData("Slider Manual Mode Used", slider.getCurrentPosition());
          //  telemetry.update();
            telemetry.addData("Slider Manual Mode Target", slider.getTargetPosition());
            telemetry.update();
            //slider.setPower(0);
           // manualSlider = false;
        }
